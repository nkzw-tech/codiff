...
Durable cache keys (`createCommitArtifactKey`, `createRangeArtifactKey`, and
`createBlobArtifactKey`) layer persistence identity above the request-local
keys. `ReviewProviderAdapter<ReviewReference, CurrentReview>` pairs a normalized
